...
# timeframes: extra TFs to CREATE OB Levels on (HTF nesting parents, 48-VISUAL G1);
# Evidence stays base-tf only -- HTF levels exist for htf_nest containment.
_DEFAULTS = {"tf": "5m", "displacement_atr": 1.5, "lookback": 3,
             "hunt_minutes": 105, "timeframes": None}
...
class OrderblockDetector(Detector):
    ...
    def detect(self, ctx: StockContext) -> list[Evidence]:
        ...
        tfs = [Timeframe(t) for t in (self.params.get("timeframes") or [])]
        last = None
        for tf in dict.fromkeys([base, *tfs]):     # base first; dedup, order kept
            atr = ctx.atr(tf)
        ...
        return self._evidence(ctx, last) if last is not None else []
    ...
    def _upsert(self, ctx: StockContext, ob: Candle, tf: Timeframe,
        ...
        rival = next((lv for lv in ctx.levels if lv.kind is kind
                      and lv.tf is tf                    # dedup within own TF only
    ...
    def _evidence(self, ctx: StockContext, last: Candle) -> list[Evidence]:
        ...
        for lv in ctx.levels:
            ...
            if (lv.kind not in _OB_KINDS or lv.state not in _LIVE
                    or lv.tf is not base      # HTF levels = nesting parents only
